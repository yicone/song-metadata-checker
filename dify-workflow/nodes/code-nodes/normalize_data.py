        ...
        for key, value in credits.items():
            ...
            if normalized_key in [normalize_string(v) for v in variants]:
                ...
                if isinstance(value, list):
                    normalized[standard_key] = sorted(
                        [normalize_string(v) for v in value]
                    )
...
def parse_spotify_data(data_str: str) -> dict:
    ...
    try:
        ...
        # 提取歌词（如果有，Spotify 官方 API 不直接提供歌词）
        ...
        return {
            ...
            "lyrics": lyrics,  # 注意：Spotify 官方 API 不提供歌词
...
def parse_qqmusic_data(data_str: str) -> dict:
    ...
    try:
        ...
        artists = normalize_artists(
            [s.get("name") for s in singers] if isinstance(singers, list) else []
        )

        # 提取歌词（如果有）
        lyrics = song_data.get("lyric", "") or song_data.get("lyrics", "")
